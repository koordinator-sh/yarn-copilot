...
function generate_import_files_pkg_map() {
  ...
  file_path="$( find ${GOYARN_API_PATH} -name ${file_name} )"

  file_pkg_map=()
  file_relative_path=${file_path##${GOYARN_ROOT}} # /pkg/yarn/apis/proto/hadoopyarn/server/resourcemanager_administration_protocol.proto
  pkg_relative_path=${file_relative_path%/*.proto} # /pkg/yarn/apis/proto/hadoopyarn/server
  file_pkg=${GOYARN_PKG_ROOT}${pkg_relative_path} # github.com/koordiantor-sh/koordiantor/pkg/yarn/apis/proto/hadoopyarn/server
...
do
    ...
    yarn_api_file_relative_path=${yarn_api_file##${GOYARN_ROOT}}
...
do
    PROTO_PATH_ARGS+="--proto_path=${GOYARN_ROOT}${yarn_api_pkg_relative_path} "
done
